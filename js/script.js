...
async function populateSuggestedProducts() {
  ...
  try {
      ...
      globalProducts.forEach(p => {
        ...
        if (finalImageUrl && !finalImageUrl.startsWith('images/')) { // nếu không phải ảnh placeholder của frontend
             if (!finalImageUrl.startsWith('http')) { // Tránh ghép lại nếu đã là URL đầy đủ
                finalImageUrl = `http://localhost:3000${finalImageUrl}`;
             }
        } else if (!finalImageUrl) { // Nếu p.image bị null/undefined
        ...
        const productHtml = `
          <div class="product-item">
            ...
            <button class="delete-btn" data-id="${p.id}" style="
...
function initializeAddProductModal() {
    ...
    addProductForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            alert(`Đã thêm: ${addedProduct.name} (ID: ${addedProduct.id})`);
